...
public class Application {
	...
	public static void main(String[] args) {
		 ...
		 * 1) port number on which client will be listening
		 * 2) server IP address
		 * 3) port number on which server application running
		 * If the number of arguments is different than 3, then
		 * IP address and port number are set to default values
		 */
		if (args.length == 3) {
		...
		Client client;
		try {
			client = new Client(localPortNumber, serverAddress, serverPortNumber);
			client.run();
